...
namespace FabRun.Api.Services
{
    public class StravaService
    {
        ...
        // ---------- OAuth ----------
        public string AuthorizeUrl(string clientId, string redirectUri, string scopeCsv = "read,activity:read_all")
        {
        ...
        {
            var after = DateTimeOffset.UtcNow.AddDays(-daysBack).ToUnixTimeSeconds();
            var all = new List<Activity>();
            int page = 1;
            while (true)
            {
                var url = $"https://www.strava.com/api/v3/athlete/activities?after={after}&per_page=200&page={page}";
        ...
        public static Kpis ComputeKpis(IEnumerable<Activity> activities)
        {
            ...
            var lastYear = runs.Where(a => DateTime.Parse(a.start_date_local) >= DateTime.UtcNow.AddDays(-365)).ToList();

            double totalKm = lastYear.Sum(a => a.distance / 1000.0);
            double totalSec = lastYear.Sum(a => (double)a.moving_time);
            double avg = totalKm > 0 ? totalSec / totalKm : 0.0;

            double best = lastYear
            ...
            double longest = lastYear.Select(a => a.distance / 1000.0).DefaultIfEmpty(0).Max();

            var weekly = new Dictionary<string, double>();
            foreach (var a in lastYear)
            {
        ...
        public async Task<Kpis> BuildKpisAsync(string accessToken, int daysBack = 365)
        {
